...
public class Graph {	
	...
	private int[][]randVisit=new int[MAX_SIZE][MAX_SIZE];
	...
	public List<List<String>>calcShortestPath(String word1,String word2){
		...
		int [][]shortestPathMartix=new int[MAX_SIZE][MAX_SIZE];
			...
			for(int i=0;i!=adjacencyList.size();i++) {
				...
				if(!temp.equals(word1)) {
					...
					if(list2.size()==0)
						...
						list2.add(0,"the cost is"+cost.get(strIndex.get(temp)));
	...
	public void randInit() {
		randStart=null;
		randEnd=null;
		randVisit=new int[MAX_SIZE][MAX_SIZE];
	}
	public String randomWalk() {
		...
		if(randStart==null) {
			...
			if(list.size()!=0)
			{
				...
				if(randVisit[strIndex.get(temp.vertex)][strIndex.get(randStart.vertex)]==1)
				{
					randEnd=randStart;
					randStart=null;
					return "";
